...
class MainWindow(QMainWindow):
    def __init__(self, archivo, channel_colors, stim_colors, values):
        ...
        # Configura el botón para realizar el análisis de STA
        self.bt_go.clicked.connect(self.analisis_STA)
    ...
    def analisis_STA(self):
        """
        Realiza un análisis de Promedio Disparado por Spike (Spike-Triggered Average, STA) en una serie de señales.
        Esta función procesa los datos de señal alrededor de eventos de spikes, normaliza las señales y genera un gráfico del promedio de la actividad de estímulo
        en cada señal alineada a los spikes detectados.

        Proceso detallado:
        - Configura etiquetas y define el rango de tiempo antes y después de los eventos de estimulación.
        - Combina las señales en una matriz y reemplaza valores NaN con 0.
        - Normaliza las señales en función de los valores mínimos y máximos especificados por el usuario.
        - Extrae segmentos de señal alrededor de cada evento de spike.
        - Calcula el promedio de estos segmentos para cada señal.
        - Crea gráficos de promedio disparado por spike para cada señal.
        - Guarda los resultados en un archivo CSV y genera una visualización de la figura con opción de guardado en imagen.

        Parámetros:
        - `self.signals_labels`: etiquetas de las señales.
        - `self.before`, `self.after`: tiempo antes y después del spike para extraer segmentos.
        - `self.analysis_on_window`: flag que indica si se analiza una ventana específica de las señales.
        - `self.signals_filtradas`: matriz de señales filtradas.
        - `self.stim`: vector con indicación de eventos de spike.
        - `self.datos_por_s`: tasa de muestreo en datos por segundo.
        - `self.save_function_flag`: flag que indica si se guardarán los resultados.

        Variables intermedias:
        - `s_antes`, `s_despues`: Tiempo en segundos antes y después de cada spike.
        - `d_antes`, `d_despues`: Número de datos en la ventana antes y después del spike.
        - `self.normalized_signal`: matriz que almacena las señales normalizadas.

        Resultados:
        - Grafica el promedio de los segmentos alrededor de spikes para cada señal.
        - Guarda los resultados de los promedios y la configuración en un archivo CSV.

        Excepciones:
        - `ValueError`: Si `max_val` o `min_val` son NaN durante la normalización.

        """
        
        print("ANÁLISIS DE STA EN CURSO...")
        ...
        # Extracción y procesamiento de datos alrededor de eventos de spikes
        stimulus_segments = []
        spike_times = self.time[self.stim == 1.0]
        
        segment_spike_times = []
        
        d_antes = round(s_antes * self.datos_por_s)
        d_despues = round(s_despues * self.datos_por_s)
        
        for spike_time in spike_times:
            start_time = round((spike_time - s_antes)* self.datos_por_s) 
            end_time = round((spike_time + s_despues)* self.datos_por_s)

            # Verificar si la ventana está dentro del rango de tiempo disponible
            if start_time >= 0 and end_time < self.normalized_signal.shape[0]:
                segment = self.normalized_signal[start_time:end_time + 1, :]
                if segment.shape[0] == d_antes + d_despues + 1:
                    stimulus_segments.append(segment)
                    segment_spike_times.append(spike_time)

        # Calcular el promedio de los segmentos para cada señal
        if stimulus_segments:
            stimulus_segments = np.array(stimulus_segments)
            averaged_stimuli = np.mean(stimulus_segments, axis=0)
        else:
            averaged_stimuli = np.array([])
        
        #Segmentos de estímulo alrededor de los spikes
        time_axis = np.linspace(-s_antes, s_despues, d_antes + d_despues + 1)

        # Crear subplots, uno para cada señal
        fig, axs = plt.subplots(len(self.signals_labels), 1, figsize=(10, 2 * len(self.signals_labels)))
        

        for idx, label in enumerate(self.signals_labels):
            """for i, segment in enumerate(stimulus_segments):
                axs[idx].plot(time_axis, segment[:, idx], label=f'Segment {i+1}', alpha=0.5)"""
            signal_color_t = getattr(self, f's_{idx + 1}')  # Obtiene el color asignado a la señal actual

            # Resultado de STA para la señal actual
            if averaged_stimuli.size > 0:
                axs[idx].plot(time_axis, averaged_stimuli[:, idx], label=f'STA {label}', color=self.color_map[signal_color_t], linewidth=2)
                axs[idx].set_xlabel('Time relative to spike (s)',  color='gray')
                axs[idx].set_ylabel(f'Stimulus ({label})')
                axs[idx].set_title(f'Spike Triggered Average - {label}', color='gray')
                axs[idx].legend()
                
            else:
                axs[idx].text(0.5, 0.5, 'No spike segments available for averaging', horizontalalignment='center', verticalalignment='center', transform=axs[idx].transAxes, color='white')
                axs[idx].set_title(f'Spike Triggered Average - {label}', color='gray')
                axs[idx].set_xlabel('Time relative to spike (s)')
                axs[idx].set_ylabel(f'STA ({label})')
                
            axs[idx].grid(True)

            axs[idx].xaxis.label.set_color('gray')  # Cambia el color de las etiquetas del eje x a gris
            axs[idx].yaxis.label.set_color('gray')  # Cambia el color de las etiquetas del eje y a gris
            axs[idx].tick_params(axis='x', colors='gray')  # Cambia el color de las marcas del eje x a gris
            axs[idx].tick_params(axis='y', colors='gray')  # Cambia el color de las marcas del eje y a gris
            
            # Configurar apariencia del subgráfico
            axs[idx].set_facecolor('none')  # Color de fondo transparente para el área del gráfico
            axs[idx].spines['left'].set_color('none')  # Ocultar espina izquierda
            axs[idx].spines['right'].set_color('none')  # Ocultar espina derecha
            axs[idx].spines['top'].set_color('none')  # Ocultar espina superior
            axs[idx].spines['bottom'].set_color('none')  # Ocultar espina inferior
            ...
            if self.save_function_flag:
                if results.empty:
                    ...
                    results['Time relative to spike (s)'] = time_axis
                
                # Crear un DataFrame temporal con los resultados
                results[f'Stimulus ({label})'] = averaged_stimuli[:, idx]
        ...
        # Guardar la figura
        plt.tight_layout()
        self.save_fig(f"{self.new_file_name}_{s_antes}s-{s_despues}s_STA")
        ...
        if self.save_function_flag:
            ...
            # Genera un timestamp y un nombre de archivo único basado en el nombre proporcionado y el timestamp actual
            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            name = f"{self.new_file_name}_STA"
            nombre_archivo_data = f"{name}_{timestamp}.csv"
            self.nombre_archivo = nombre_archivo_data  # Guarda el nombre del archivo para su uso posterior
            # Ruta completa del archivo
            file_path = os.path.join(self.carpeta_datos, nombre_archivo_data)
        ...
        #return segment_spike_times, stimulus_segments, averaged_stimuli
