...
class MainWindow(QMainWindow):
    def __init__(self, archivo, channel_colors, stim_colors, values):
        ...
        self.num_dominant_button.valueChanged.connect(self.num_dominant_button_update)

        # Configura el botón para realizar el análisis de FFT
        self.bt_go.clicked.connect(self.analisis_FFT)
    ...
    def num_dominant_button_update(self, value):
        """Actualiza el número de dominantes por señal.

        Parámetros:
            value (float): Nuevo valor para el número de dominantes por señal."""
        self.num_dominant = value  # Actualiza el número de dominantes por señal.
    ...
    def analisis_FFT(self):
        """
        Realiza el análisis de la Transformada Rápida de Fourier (FFT) en las señales 
        filtradas, graficando el espectro de frecuencias de cada una de ellas y 
        destacando las frecuencias dominantes. Este análisis permite obtener información 
        sobre las componentes frecuenciales de las señales, útiles para detectar patrones 
        periódicos o características específicas en el dominio de la frecuencia.
        
        La FFT transforma una señal del dominio del tiempo al dominio de la frecuencia 
        y calcula la magnitud de las frecuencias presentes en la señal.

        Parameters:
        - self.signals_labels: list
            Lista de etiquetas de las señales a analizar.
        - self.signals_filtradas: numpy array
            Matriz que contiene las señales filtradas en columnas, cada columna corresponde 
            a una señal diferente.
        - self.analysis_on_window: bool
            Si es True, realiza el análisis de FFT en una ventana específica de la señal 
            (del rango start_a_num a end_a_num). Si es False, analiza la señal completa.
        - self.start_a_num: int
            Índice inicial de la ventana de análisis si analysis_on_window es True.
        - self.end_a_num: int
            Índice final de la ventana de análisis si analysis_on_window es True.
        - self.num_dominant: int
            Número de frecuencias dominantes que se desea resaltar en el espectro de 
            frecuencias.
        - self.save_function_flag: bool
            Si es True, guarda los resultados de la FFT.

        Returns:
        - fft_results: pandas DataFrame
            DataFrame que contiene las frecuencias filtradas y las magnitudes de la FFT para 
            cada señal analizada.
        
    """
    
        print("ANÁLISIS DE FFT EN CURSO...")
        ...
        dominant_frequencies_dict = {}
        num_dominant = [self.num_dominant] * num_signals
        ...
        T = self.time[1] - self.time[0]  # Intervalo de muestreo
        ...
        for i in range(num_signals):
            signal = self.signals_filtradas[:, i]
            column = self.signals_labels[i]
            yf = np.fft.fft(signal)  # FFT de la señal
            xf = np.fft.fftfreq(N, T)[:N // 2]  # Frecuencias correspondientes
            
            # Filtrar frecuencias dentro del rango deseado
            indices = np.where((xf > 0) & (xf <= 5))  # Índices de frecuencias dentro de 0 a 4.5 Hz
            xf_filtered = xf[indices]
            yf_filtered = yf[indices]
            
            yf_magnitude = 2 / N * np.abs(yf_filtered[0:N // 2])# Escalar y tomar la magnitud de las componentes positivas
            axes[i].set_xlabel('Frecuencia (Hz)')
            axes[i].set_ylabel('Amplitud')
            axes[i].set_title(f'FFT de {column}', color='gray')
            axes[i].grid(True)
            
            signal_color_t = getattr(self, f's_{index_prom_tag + 1}')  # Obtiene el color asignado a la señal actual
            axes[i].plot(xf_filtered, yf_magnitude,
                                            label=f'FFT de CH{index_prom_tag + 1}', color=self.color_map[signal_color_t])
            ...
            # Encontrar los índices de los picos en la FFT
            #peaks_indices = np.argsort(np.abs(yf_filtered))[-num_d:]
            peaks_indices = np.argsort(np.abs(yf_filtered))[-num_dominant[i]:]
            
            # Seleccionar las frecuencias dominantes
            #dominant_frequencies = xf_filtered[peaks_indices]
            dominant_frequencies = xf_filtered[peaks_indices][::-1]

            # Guardar las frecuencias dominantes en el diccionario
            #dominant_frequencies_dict[column] = dominant_frequencies
            dominant_frequencies_dict[column] = dominant_frequencies
            
            dominant_magnitudes = yf_magnitude[peaks_indices][::-1]

            # Añadir líneas verticales para las frecuencias dominantes con la altura correspondiente
            for j, (freq, mag) in enumerate(zip(dominant_frequencies, dominant_magnitudes)):
                color = colors[j % len(colors)]  # Elegir el color según el orden
                axes[i].axvline(x=freq, ymin=0, ymax=mag/max(yf_magnitude), color=color, linestyle='--', label=f'{freq} Hz ({mag})')

            # Añadir leyenda
            # axes[i].legend()
            
            # fig.patch.set_facecolor('black')      
            # axes[i].set_facecolor('black')  # Cambia el color de fondo de cada gráfico individual a negro
            # axes[i].xaxis.label.set_color('gray')  # Cambia el color de las etiquetas del eje x a gris
            # axes[i].yaxis.label.set_color('gray')  # Cambia el color de las etiquetas del eje y a gris
            # axes[i].tick_params(axis='x', colors='gray')  # Cambia el color de las marcas del eje x a gris
            # axes[i].tick_params(axis='y', colors='gray')  # Cambia el color de las marcas del eje y a gris
            
            """ # Guardar cada gráfica individualmente
            individual_fig, individual_ax = plt.subplots()
            individual_ax.plot(xf_filtered, yf_magnitude)
            individual_ax.set_xlabel('Frecuencia (Hz)')
            individual_ax.set_ylabel('Amplitud')
            individual_ax.set_title(f'FFT de {column}')
            ...
            for j, (freq, mag) in enumerate(zip(dominant_frequencies, dominant_magnitudes)):
                color = colors[j % len(colors)]  # Elegir el color según el orden
                individual_ax.axvline(x=freq, ymin=0, ymax=mag/max(yf_magnitude), color=color, linestyle='--', label=f'{freq} Hz ({mag})')
            ...
            self.save_fig(f"{self.new_file_name}_CH{index_prom_tag + 1}_FFT")
            #individual_fig.savefig(f'FFT_{column}.svg', format='svg')
        ...
        # Guardar la figura
        plt.tight_layout()
        self.save_fig(f"{self.new_file_name}_FFT")
        ...
        if self.save_function_flag:
            ...
            # Genera un timestamp y un nombre de archivo único basado en el nombre proporcionado y el timestamp actual
            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            name = f"{self.new_file_name}_FFT"
            nombre_archivo_data = f"{name}_{timestamp}.csv"
            self.nombre_archivo = nombre_archivo_data  # Guarda el nombre del archivo para su uso posterior
            # Ruta completa del archivo
            file_path = os.path.join(self.carpeta_datos, nombre_archivo_data)
            fft_results.to_csv(file_path, index=False)
            print(f"Datos guardados en {file_path}")

        
        print(dominant_frequencies_dict)

        return dominant_frequencies_dict
