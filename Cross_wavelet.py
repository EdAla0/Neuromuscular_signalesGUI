...
class MainWindow(QMainWindow):
    def __init__(self, archivo, channel_colors, stim_colors, values):
        ...
        # Configura el botón para realizar el análisis de Cross_wavelet
        self.bt_go.clicked.connect(self.Cross_wavelet)
        ...
        self.mother = 'fbsp7-1.5-1.0' #mother: La mother wavelet que se usará. Más información sobre cada una en: https://pywavelets.readthedocs.io/en/latest/ref/wavelets.html
        ...
        self.before = 1
        self.after = 1
        self.new_file_name = "Cross_wavelet"
    ...
    def Cross_wavelet(self):
        """
        Realiza el análisis de correlación cruzada de wavelets (Cross-Wavelet) entre dos señales.

        La función calcula el espectro de correlación cruzada entre dos señales usando la transformada 
        wavelet continua (CWT). Los parámetros de la función permiten configurar la normalización, 
        eliminación de tendencia, factor de submuestreo, y el tipo de wavelet madre. Los resultados 
        incluyen un gráfico del espectro de correlación cruzada en función del tiempo y la frecuencia, 
        además de guardar un archivo CSV con los resultados detallados.

        Parámetros:
        - `nNotes` (int): Número de notas para el cálculo de escalas.
        - `detrend` (bool): Indica si se elimina la tendencia lineal de las señales.
        - `normalize` (bool): Indica si se normalizan las señales antes del análisis.
        - `subsample_factor` (int): Factor de submuestreo para reducir el tamaño de las señales.
        - `mother` (str): Nombre de la wavelet madre utilizada en el análisis.
        - `analysis_on_window` (bool): Si True, realiza el análisis en un intervalo específico de tiempo.

        Retorna:
        - `correlation_wavelet` (ndarray): Matriz de correlación cruzada de wavelets.
        - `times` (ndarray): Vector de tiempos de la señal.
        - `frequencies` (ndarray): Vector de frecuencias en Hz correspondientes a las escalas de wavelet.
        - `coif` (ndarray): Cono de influencia que limita las zonas válidas en el espectro.


        Notas:
        - La función reemplaza valores `NaN` en las señales con ceros y permite graficar el 
        espectro de correlación cruzada junto con el cono de influencia.
        - El archivo CSV generado contiene información detallada del análisis, incluyendo la 
        frecuencia dominante y valores promedio del espectro de correlación.
        """

        
        print("ANÁLISIS DE Cross_wavelet EN CURSO...")
        ...
        correlation_wavelet = np.abs(coef1 * np.conj(coef2))
        ...
        def summarize_xcorrelation(xcorrelation, frequencies, times):
            """
            Proporciona un resumen numérico de la correlación cruzada de wavelets.
            ...
            Returns:
            - summary (dict): Resumen de correlación cruzada de wavelets con valores promedio y frecuencia dominante.
            """
            avg_xcorrelation = np.mean(xcorrelation)
            max_xcorrelation_freq_index = np.argmax(np.mean(xcorrelation, axis=1))
            max_xcorrelation_freq = frequencies[max_xcorrelation_freq_index]
            
            summary = {
                'average_xcorrelation': avg_xcorrelation,
                'dominant_frequency': max_xcorrelation_freq,
                'xcorrelation_matrix': xcorrelation
            }

            return summary
        xcorrelation_summary = summarize_xcorrelation(correlation_wavelet, frequencies, times)
        print(xcorrelation_summary)

        # Imprimir el resumen
        print("Resumen de Cross-wavelet spectrum:")
        print(f"Promedio de cross-wavelet spectrum: {xcorrelation_summary['average_xcorrelation']:.4f}")
        print(f"Frecuencia Dominante: {xcorrelation_summary['dominant_frequency']:.4f} Hz")
        ...
        spectrogram_plot(correlation_wavelet, times, frequencies, coif, cmap='turbo', ax=ax)
        ax.set_xlabel('Tiempo (s)')
        ax.set_ylabel('Frecuencia (Hz)')
        ax.set_title(f'Cross-wavelet spectrum {self.s_1} - {self.s_2}', color='gray')
        ...
        plt.tight_layout()
        self.save_fig(f"{self.new_file_name}_Cross_Wavelet")
        ...
        if self.save_function_flag:
            ...
            # Crear un DataFrame con la matriz de correlation_wavelet
            results = pd.DataFrame(correlation_wavelet, index=frequencies, columns=times)
            ...
            name = f"{self.new_file_name}_Cross-wavelet_{self.s_1} - {self.s_2}"
        ...
        return correlation_wavelet, times, frequencies, coif
