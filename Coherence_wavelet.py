...
class MainWindow(QMainWindow):
    def __init__(self, archivo, channel_colors, stim_colors, values):
        ...
        loadUi('Wavelet_coherence.ui', self)
        ...
        # Configura el botón para realizar el análisis de Wavelet_coherence
        self.bt_go.clicked.connect(self.Wavelet_coherence)
        ...
        self.before = 1
        self.after = 1
        self.new_file_name = "Wavelet_coherence"
    ...
    def Wavelet_coherence(self):
        """
        Realiza el análisis de coherencia wavelet entre dos señales utilizando la 
        Transformada Wavelet Continua (CWT) y calcula su coherencia.

        Este método permite analizar cómo varía la relación de frecuencia y fase 
        entre dos señales a lo largo del tiempo, lo cual es útil en estudios de 
        interacciones entre series temporales.

        Attributes:
            nNotes (int): Número de notas o resoluciones de la transformada wavelet.
            detrend (bool): Indica si se debe eliminar la tendencia lineal de las señales.
            normalize (bool): Indica si se debe normalizar las señales antes del análisis.
            subsample_factor (int): Factor para reducir el número de muestras (submuestreo).
            mother (str): Tipo de wavelet madre utilizada para la transformada.
            signals_labels (list): Etiquetas de las señales a analizar.

        Returns:
            Esta función genera gráficas de coherencía.
        """
        
        print("ANÁLISIS DE Wavelet_coherence EN CURSO...")
        ...
        def summarize_CWT(CWT, frequencies, times):
            """
            ...
            """
            avg_CWT = np.mean(CWT)
            max_CWT_freq_index = np.argmax(np.mean(CWT, axis=1))
            max_CWT_freq = frequencies[max_CWT_freq_index]
            
            summary = {
                'average_CWT': avg_CWT,
                'dominant_frequency': max_CWT_freq,
                'CWT_matrix': CWT
            }

            return summary
        CWT_summary = summarize_CWT(CWT, frequencies, times)
        print(CWT_summary)

        # Imprimir el resumen
        print("Resumen de Wavelet coherence:")
        print(f"Promedio de Wavelet coherence: {CWT_summary['average_CWT']:.4f}")
        print(f"Frecuencia Dominante: {CWT_summary['dominant_frequency']:.4f} Hz")
        # Graficar la correlación cruzada y guardar la figura como SVG
        fig, ax = plt.subplots()
        # Calcula la fase de la CWT
        phase_data = np.angle(CWT)
        phase_data[CWT <= 0.90] = np.nan
        ...
        spectrogram_plot(CWT, times, frequencies, coif, cmap='turbo',norm = Normalize(), ax=ax, phase_data=phase_data, arrow_density=arrow_density)
        ax.set_xlabel('Tiempo (s)')
        ax.set_ylabel('Frecuencia (Hz)')
        ax.set_title('Wavelet coherence', color='gray')
        ...
        plt.tight_layout()
        self.save_fig(f"{self.new_file_name}_Wavelet_coherence")
        ...
        if self.save_function_flag:
            ...
            # Crear un DataFrame con la matriz de coherencia wavelet
            results = pd.DataFrame(CWT, index=frequencies, columns=times)
            ...
            name = f"{self.new_file_name}_CWT_{self.s_1} - {self.s_2}"
